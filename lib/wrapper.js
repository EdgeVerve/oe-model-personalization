    ...
    if (context && !utils2.isDefaultContext(autoscopeFields, context) && !m.filebased) {
      ...
      if (m.relations) {
        Object.keys(m.relations).forEach(function (item) {
          ...
          if (r.clientModel && !app.models[r.clientModel]) {
            r.model = utils.createModelId(r.clientModel, autoscopeFields, context);
          }
          if (r.through && !app.models[r.through]) {
            r.throughClientModel = r.through;
            r.through = utils.createModelId(r.throughClientModel, autoscopeFields, context);
          }
        });
      }
